"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.BrandsResolver = void 0;
const graphql_1 = require("@nestjs/graphql");
const brands_service_1 = require("./brands.service");
const brands_schema_1 = require("./entities/brands.schema");
const brands_input_1 = require("./dto/brands.input");
const paginated_brands_output_1 = require("./dto/paginated-brands.output");
let BrandsResolver = class BrandsResolver {
    constructor(brandsService) {
        this.brandsService = brandsService;
    }
    findAll() {
        return this.brandsService.findAll();
    }
    createBrand(createBrandInput) {
        return this.brandsService.create(createBrandInput);
    }
    updateBrand(brandId, updateBrandInput) {
        return this.brandsService.updateByBrandId(brandId, updateBrandInput);
    }
    removeBrand(brandId) {
        return this.brandsService.removeByBrandId(brandId);
    }
    brandsPaginated(page, limit, search, status, startDate, endDate) {
        return this.brandsService.findPaginated(page, limit, search, status, startDate, endDate);
    }
};
exports.BrandsResolver = BrandsResolver;
__decorate([
    (0, graphql_1.Query)(() => [brands_schema_1.Brand], { name: 'brands' }),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", void 0)
], BrandsResolver.prototype, "findAll", null);
__decorate([
    (0, graphql_1.Mutation)(() => brands_schema_1.Brand),
    __param(0, (0, graphql_1.Args)('createBrandInput')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [brands_input_1.CreateBrandInput]),
    __metadata("design:returntype", void 0)
], BrandsResolver.prototype, "createBrand", null);
__decorate([
    (0, graphql_1.Mutation)(() => brands_schema_1.Brand),
    __param(0, (0, graphql_1.Args)('brandId', { type: () => graphql_1.Int })),
    __param(1, (0, graphql_1.Args)('updateBrandInput')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number, brands_input_1.CreateBrandInput]),
    __metadata("design:returntype", void 0)
], BrandsResolver.prototype, "updateBrand", null);
__decorate([
    (0, graphql_1.Mutation)(() => brands_schema_1.Brand),
    __param(0, (0, graphql_1.Args)('brandId', { type: () => graphql_1.Int })),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number]),
    __metadata("design:returntype", void 0)
], BrandsResolver.prototype, "removeBrand", null);
__decorate([
    (0, graphql_1.Query)(() => paginated_brands_output_1.PaginatedBrands),
    __param(0, (0, graphql_1.Args)('page', { type: () => graphql_1.Int })),
    __param(1, (0, graphql_1.Args)('limit', { type: () => graphql_1.Int })),
    __param(2, (0, graphql_1.Args)('search', { type: () => String, nullable: true })),
    __param(3, (0, graphql_1.Args)('status', { type: () => String, nullable: true })),
    __param(4, (0, graphql_1.Args)('startDate', { type: () => String, nullable: true })),
    __param(5, (0, graphql_1.Args)('endDate', { type: () => String, nullable: true })),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number, Number, String, String, String, String]),
    __metadata("design:returntype", void 0)
], BrandsResolver.prototype, "brandsPaginated", null);
exports.BrandsResolver = BrandsResolver = __decorate([
    (0, graphql_1.Resolver)(() => brands_schema_1.Brand),
    __metadata("design:paramtypes", [brands_service_1.BrandsService])
], BrandsResolver);
//# sourceMappingURL=brands.resolver.js.map